...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text || !text.includes('|')) {
    return m.reply(
      `🚩 Incorrect format.\n` +
      `Example: ${usedPrefix}${command} 𝐶𝑟𝑎𝑧𝑦 ouafy| +201214057674 | Busy\n` +
  ...
  try {
    ...
    if (m.quoted && m.quoted.mtype === 'imageMessage') {
      ...
      try {
        ...
        profilePicUrl = 'https://i.postimg.cc/2jFJGwzS/IMG-20260610-WA0072.jpg'; // Default avatar
      }
    }
    ...
    const [avatar, background] = await Promise.all([
      ...
      loadImage('https://i.postimg.cc/gksCzK5n/IMG-20260610-WA0076.jpg') // Background template
    ]);
...
handler.help = ['واتساب-وهمي'];
handler.tags = ['tools'];
handler.command = /^واتساب-وهمي$/i;
handler.limit = true;
