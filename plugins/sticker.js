...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (/webp/g.test(mime)) {
      ...
      //	if ((q.msg || q).seconds > 10) throw 'الحد الأقصى 10 ثوانٍ!'
      let img = await q.download?.();
      ...
      throw `أرسل أو رد على صورة/فيديو/ملصق بالأمر ${usedPrefix + command}`;
  } catch (e) {
    console.log(e);
    stiker = e;
  } finally {
...
handler.help = ["ملصق-كامل", "ملصق", "ملصق-gif"];
handler.tags = ["sticker"];
handler.command = /^(ملصق-كامل|ملصق-gif|م|ملصق|ملصق2)$/i;
handler.limit = 1
