// م2.js - قـسـم الـتـحـمـيـل (DOWNLOAD) 🍒
// تم التعديل لاستخدام روابط سريعة وإرسال صورة/فيديو مثل قسم الإدارة
...
let handler = async (m, { conn, usedPrefix }) => {
  // 1. مصفوفة الروابط السريعة (مثل القسم السابق)
  const assets = [
  ...
  // 3. جمع أوامر قسم التحميل (download / downloader / تحميل)
  let downloadCommands = [];
  if (global.plugins) {
    for (let plugin of Object.values(global.plugins)) {
      if (!plugin) continue;
      let isDownloadCmd = false;
      if (plugin.tags && Array.isArray(plugin.tags)) {
        if (plugin.tags.includes('download') || plugin.tags.includes('downloader') || plugin.tags.includes('تحميل')) {
          isDownloadCmd = true;
        }
      }
      if (isDownloadCmd) {
        let cmds = extractCommands(plugin, usedPrefix);
        downloadCommands.push(...cmds);
      }
    }
  }
  downloadCommands = [...new Set(downloadCommands)].sort();
...
𓆩⃞⚙️𓆪  قـسـم الـتـحـمـيـل (DOWNLOAD) 🧩
╯──────────────╰

╮───≪ ⚙️ الأوامر المتاحة≫───╭`.trim();

  if (downloadCommands.length === 0) {
    menuText += `\n│ ⌬ لا توجد أوامر تحميل حالياً`;
  } else {
    for (let cmd of downloadCommands) {
      menuText += `\n│ ⌬ ${cmd}`;
    }
  }
...
`.trim();
  ...
  // 4. تجهيز التوجيه (forwardingScore = 1 لتجنب الإزعاج)
  const contextInfo = {
