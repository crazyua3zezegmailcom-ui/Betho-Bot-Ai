let handler = async (m, { conn, text, usedPrefix, command }) => {
  try {
    ...
    const tagList = Object.entries(counts)
      .sort((a, b) => b[1] - a[1])
      .map(([tag, count]) => `тнФ ${(tag.charAt(0).toUpperCase() + tag.slice(1)).padEnd(13)} - ${count.toString().padStart(3)}`)
      .join('\n');

    const totalCommands = Object.values(counts).reduce((a, b) => a + b, 0);
    const responseText = "```" + `${tagList}\n` + "```";

    await conn.reply(m.chat, `*[ Betho Bot BOT FEATURES LIST ]*\n\n${responseText}\n\n*Total feature: ${totalCommands} Commands*`, m);
  } catch (error) {
    console.error(error);
...
handler.tags = ['tools']
handler.command = /^(الميزات|الميزات2)$/i
