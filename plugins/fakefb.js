...
let handler = async (m, { text, conn, usedPrefix, command }) => {
  try {
    ...
    if (!name || !comment) throw `Invalid format!\n\nExample: ${usedPrefix + command} 𝐶𝑟𝑎𝑧𝑦 Ouafy|lbot smito betho!`

    let ppUrl = await conn.profilePictureUrl(m.sender, 'image').catch(() => 'https://i.postimg.cc/5NkJJV6H/IMG-20260610-WA0080.jpg')
...
handler.help = ['فيس-وهمي']
handler.tags = ['tools']
handler.command = /^فيس-وهمي$/i
handler.limit= true
