...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    // الهيدرز المنسخة بدقة 100% من الـ curl الجديد الخاص بك
    const headers = {
    ...
    // الـ Payload الدقيق جداً مع الـ bot_uid و use_model الخاصين بـ gpt-4.1-nano
    const payload = {
    ...
    try {
        ...
        // معالجة تدفق البيانات المباشر (SSE) بدقة
            ...
            for (let line of lines) {
                if (line.startsWith('data: ')) {
                    ...
                    try {
                        ...
                        // تفادي أي تعليق أو خطأ في الـ chunk المتناثر
                    }
                }
            }
        });

        // إرسال الرد بمجرد انتهاء البث بنجاح
