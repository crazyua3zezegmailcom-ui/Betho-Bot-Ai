  ...
  if (global.plugins) {
    for (let plugin of Object.values(global.plugins)) {
      ...
      if (plugin.tags && Array.isArray(plugin.tags)) {
        if (plugin.tags.includes('search') || plugin.tags.includes('بحث') || plugin.tags.includes('engine')) {
          isSearchCmd = true;
        }
      }
      if (isSearchCmd) {
        let cmds = extractCommands(plugin, usedPrefix);
        searchCommands.push(...cmds);
      }
    }
  }
  searchCommands = [...new Set(searchCommands)].sort();
...
╮───≪ 🌴 الأوامر المتاحة≫───╭`.trim();
  ...
  if (searchCommands.length === 0) {
    menuText += `\n│ ⌬ لا توجد أوامر بحث حالياً`;
  } else {
    for (let cmd of searchCommands) {
      menuText += `\n│ ⌬ ${cmd}`;
    }
  }
