...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    const groupMetadata = m.isGroup ? { ..._rawMeta, ...(_rawMeta.participants && { participants: (_rawMeta.participants || []).map(p => ({ ...p, id: p.jid, jid: p.jid, lid: p.lid })) }) } : {};
        ...
        if (chat) {
          ...
          if (name !== "group-banchat.js" && chat?.isBanned && !isROwner) {
            if (!primaryBotId || primaryBotId === botId) {
              const aviso = `⚠️ البوت معطل في هذه المجموعة.\nيمكن لأحد المشرفين تفعيله باستخدام الأمر: ${usedPrefix}bot on`;
              await m.reply(aviso);
              return;
            }
          }
          if (m.text && user.banned && !isROwner) {
            const mensaje = `❌ أنت محظور من استخدام هذا البوت.\nالسبب: ${user.bannedReason}`;
            if (!primaryBotId || primaryBotId === botId) {
              m.reply(mensaje);
              return;
            }
...
global.dfail = (type, m, conn) => {
  const msg = {
    ...
    premium: `${startDeco}\n\n⭐ هذا الأمر متاح فقط للمستخدمين المميزين (Premium).\n\n${endDeco}`,
    group: `${startDeco}\n\n📌 هذا الأمر يعمل فقط داخل المجموعات.\n\n${endDeco}`,
    private: `${startDeco}\n\n🔒 هذا الأمر يعمل فقط في الخاص مع البوت.\n\n${endDeco}`,
    admin: `${startDeco}\n\n🛠️ هذا الأمر خاص بمشرفي المجموعة فقط.\n\n${endDeco}`,
...
watchFile(file, async () => {
  unwatchFile(file);
  console.log(chalk.magenta("تم تحديث ملف handler.js"));
